/* Link hubs to hobbies and store display fields for hub profile pages. */
...
COMMENT ON COLUMN public.hubs.gradient_from IS 'Left/top hex color for banner gradient';
COMMENT ON COLUMN public.hubs.gradient_to IS 'Right/bottom hex color for banner gradient';

/* Required hobby rows — seed if missing (hub rows depend on these slugs) */
...
/* Seed / upsert three hubs tied to hobbies by slug */
